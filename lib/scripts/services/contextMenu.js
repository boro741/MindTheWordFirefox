...
export class ContextMenu {
  ...
  addWordToBlacklist(wordToBeBlacklisted){
    ...
    browser.storage.local.get('userBlacklistedWords', function(result){
  ...
  addToDifficultyBucket(word, difficultyLevel, translatedWords){
    ...
    if (isTranslatedWord){
      ...
      browser.storage.local.get(['difficultyBuckets'], (result) => {
  ...
  markWordAsLearnt(word, translatedWords){
    ...
    if (isTranslatedWord){
      browser.storage.local.get(['learntWords'], function(result){
  ...
  translateSentence(selectedText, translatedWords){
    var promise = new Promise((resolve, reject) => {
      ...
      if (words.length >= 2 && words.length < 14) {
        ...
        browser.storage.local.get(keys, (data) =>{
          var promise = {};
  ...
  removeFromDifficultyBucket(word){
    ...
    browser.storage.local.get(['difficultyBuckets'], (result) => {
      ...
      browser.storage.local.set({'difficultyBuckets': JSON.stringify(difficultyBuckets)});
    });
  }

  saveTranslationToDictionary(selectedText, translatedWords){
    browser.storage.local.get(['savedTranslations'], function(result){
