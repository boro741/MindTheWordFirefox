...
export class ContentScript {
  ...
  processTranslations(translationMap, userDefinedTMap) {
    ...
    browser.storage.local.get(['savedPatterns'], function(result){
  ...
  toggleElement(event, wordToggles, autoBlacklist) {
    ...
    if (autoBlacklist) {
      browser.storage.local.get('toggleFrequency', (data) => {
        data = JSON.parse(data.toggleFrequency);
        if (original in data) {
          if (data[original] > wordToggles) {
            // add word to blacklist
            browser.storage.local.get('userBlacklistedWords', (result) => {
